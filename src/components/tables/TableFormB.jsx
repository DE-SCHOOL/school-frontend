...
function TableFormB({ styles, tableData, header, paggingNum, tableType = '' }) {
	//declaring state variables
	const [isSortedBy, setIsSortedBy] = useState('');
	const [formBData, setFormBData] = useState(tableData);
	...
	if (tableData.length === formBData.length) {
		DATA_CONST = formBData;
	} else {
		DATA_CONST = tableData;
	}
	const handleSort = (field, fieldOpt = undefined) => {
		sortArrayObject(DATA_CONST, setFormBData, setIsSortedBy, field, fieldOpt);
	};
	if (tableData.length === formBData.length) {
		DATA_CONST = formBData;
	} else {
