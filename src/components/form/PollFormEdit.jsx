...
function PollFormEdit({ styles, choiceCount }) {
	...
	useEffect(() => {
		...
		dispatch(getQuestion({ id: params?.id }));
	}, [dispatch, params?.id]);
			...
			<div className="form">
							...
							categories?.map((cat, index) => {
								...
								return (
									<option
										key={cat?._id}
										value={cat?._id}
										selected={cat?.id === questions.question?.category?._id}
									>
										{cat?.name}
									</option>
								);
				...
				<div className="form-item">
					...
					<input
						...
						defaultValue={questions.question?.name}
					/>
				</div>
