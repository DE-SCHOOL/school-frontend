...
function FormBsForm({ styles, type = '' }) {
	...
	const formB = useSelector((state) => state.formB);
		...
		>
			<div className="form">
				...
				<div className="form-item mg-top form-file">
					<span className="file-error">{formBsData.fileError}</span>
				</div>
						...
						>
							{formBsData.file === '' ? (
								...
								<span className="file-info">
									{formBsData.file?.name.toUpperCase()}
								</span>
							)}
						</div>
					</Uploader>
			...
			{formB.error === true && formB.errorMessage && (
				<Failure message={formB.errorMessage} />
			)}
			{formB.uploadSuccess === true && <Success />}
			{/* {studentss.error === false && setStaffData(defaultInfo)} */}
			{formB.isLoadingFormBs && <Loader />}
		</form>
	);
}

export default FormBsForm;
